...
if (isset($_SESSION['admin']['logintype'])) {
    $res = $this->common->accessrecord('user_permission', ['is_view,is_add,is_edit,is_delete'], ['user_type' => 'Provider', 'user_id' => $_SESSION['admin']['id'], 'module_name' => 'Facilitator'], 'row_array');
} else {
    $res = array();
    if (empty($res)) {
        $logintype = 'main-user';
            ...
            <div class="col-lg-12 mb-1">
                <div class="card">
                    <div class="card-header">
                        <h3 class="h6 text-uppercase mb-0">All Facilitators List</h3>
                    </div>
                    <div class="card-body">
                        <table class="table table-striped table-bordered table-responsive" style="width:100%">
                            ...
                            <tbody>
                                ...
                                if (!empty($facilitator)) {
                                    $i = 1;
                                    foreach ($facilitator as $key => $row) {
                                        ...
                                        $municipality = $this->common->accessrecord('municipality', [], ['id' =>  $row->municipality], 'row');
                                                ...
                                                $municipality_name = $municipality ? $municipality->municipality : '';
                                        ...
                                        <tr id="del-<?= $row->id ?>">
                                            ...
                                            <td>
                                                ...
                                                ?>
                                                            <div class="text-center"><?= $value['acreditations'] ?>
                                                                <img src="<?= BASEURL . 'uploads/facilitator/acreditationsfiles/' . $value['acreditations_file'] ?>" width=" 50px" height="50px">
                                                                <!-- <img src="<?= BASEURL . 'uploads/facilitator/acreditationsfiles/' . $value['acreditations_file'] ?>" width=" 50px" height="50px"> -->
                                                            </div>
                                                <?php }
                                                    }
                                    ...
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                                ...
                                                <?php if ($res['is_edit'] == 1) { ?>
                                                    <a href="create-facilitator-user?id=<?= $row->id ?>" class="btn btn-info btn-sm"><i class="fa fa-edit"></i></a>
                                                <?php }
                                                if ($res['is_delete'] == 1) { ?>
                                                    <a href="javascript:;" class="btn btn-danger btn-sm" style="margin:5px 0 0" onclick="providerdeletedataFacilitator('facilitator&behalf','id','<?= $row->id ?>','<?= $trainer_nm ?>')"><i class="fa fa-trash"></i></a>
                                                <?php } ?>
                                            </td>
                                        </tr>
                                <?php $i++;
...
<script>
    function providerdeletedataFacilitator(tablename, columnname, id, training_provider) {
            ...
            function(isConfirm) {
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        ...
                        url: "providerdeletedataFacilitator?table=" + tablename + "&behalf=" + columnname + "&data=" + id + "&training_provider=" + training_provider,
